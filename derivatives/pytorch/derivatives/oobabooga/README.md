...
1. [About the Oobabooga Image](#about-the-oobabooga-image)
    - [Upgrading Oobabooga](#upgrading-Oobabooga)
...
This Oobabooga Text Generation WebUI image is built and maintained by Vast.ai. It contains development libraries to enable building custom extensions and add-ons. Please see the associated documentation below for configuration details.

The installation directory is `/workspace/text-diffusion-webui`, where you can download models and extensions.

By default, both the WebUI and API will be started.  You can modify this behavior through the `OOBABOOGA_ARGS` environment variable.

### Upgrading Oobabooga

Oobabooga can be upgraded by checking out a new version with git, installing the requirements and then restarting:

```bash
cd /workspace/text-diffusion-webui
git checkout main
git fetch --tags
git checkout [desired_ref]
pip install -r requirements.txt
supervisorctl restart oobabooga
```
...
1. On the __destination__ instance:
   ...
   - Run: `cd /workspace/ && rm -rf text-diffusion-webui`
...
| Oobabooga UI | 7860 | 17860 |
| Oobabooga API | 5000 | 15000 |
...
#### Example: Forwarding Oobabooga UI & API to localhost

To forward Oobabooga to your local machine:

```bash
ssh root@INSTANCE_IP -p SSH_PORT -L 5000:localhost:15000 -L 7860:localhost:17860
```

This command:

- Creates SSH local port forwards for your localhost:5000 and localhost:7860
- Connects to the instance internal Oobabooga ports (17860 & 15000)
- Allows you to access the applications at http://localhost:7860 http://localhost:5000 and  on your machine
...
| `OOBABOOGA_ARGS` | string | `--listen-port 17860 --api --api-port 15000` | Startup arguments to pass to Oobabooga |
...
- [Image Source](https://github.com/vast-ai/base-image/tree/main/derivatives/pytorch/derivatives/oobabooga)
