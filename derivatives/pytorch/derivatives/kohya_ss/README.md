# Kohya's GUI (kohya_ss) 

> This GUI allows you to set training parameters and generate and run the required CLI commands to train image generation models.


## Contents

1. [About the Kohya_ss Image](#about-the-kohyas-gui-image)
    - [Upgrading Kohya_ss](#upgrading-kohyas-gui)
...
## About the Kohya's GUI Image

This Kohya's GUI (kohya_ss) container image is built and maintained by Vast.ai. It is based upon the [vastai/pytorch](https://github.com/vast-ai/base-image/tree/main/derivatives/pytorch) docker image and contains development libraries to enable building custom extensions and add-ons. Please see the associated documentation below for configuration details.

The installation directory is `/workspace/kohya_ss`, where you can download models and extensions.

No models are included in the Docker image.

### Upgrading Kohya's GUI

Kohya_ss can be upgraded by checking out a new version with git, installing the requirements and then restarting:

```bash
cd /workspace/kohya_ss
git checkout master
git fetch --recurse-submodules
git checkout --recurse-submodules [desired_ref]
pip install -r requirements_linux.txt # Optional - May replace your PyTorch version
pip install -r requirements.txt
supervisorctl restart kohya
```
...
1. On the __destination__ instance:
   ...
   - Run: `cd /workspace/ && rm -rf kohya_ss`
...
#### Example: Forwarding Kohya's GUI to localhost

To forward Kohya_ss to your local machine:
...
- Connects to the instance internal Kohya_ss port (17860)
...
| `KOHYA_ARGS` | string | `--server_port 17860 --headless --noverify` | Startup arguments to pass to Kohya's GUI |
...
- [Image Source](https://github.com/vast-ai/base-image/tree/main/derivatives/pytorch/derivatives/kohya_ss)
